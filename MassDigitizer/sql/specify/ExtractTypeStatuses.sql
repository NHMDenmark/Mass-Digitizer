  -- To be run on Specify database for each collection 
  	SELECT -- pli.* -- , -- pli.Title  -- pli.PickListItemID, pli.Value, pli.Title  
			CONCAT ("(", pli.PickListItemID, ",'", pli.Title, "','", pli.Value, "',", pli.Ordinal, ", (SELECT id FROM collection WHERE spid = ", pl.CollectionID, " AND institutionid = 1)),") 
			-- Adjust institutionid : NHMD - 1, NHMA - 2  
			sqlstatement   
 FROM picklist pl 
 	JOIN picklistitem pli ON pli.PickListID = pl.PickListID 
 		WHERE pl.Name = "TypeStatus" AND pl.CollectionID = 327682 -- Adjust collection id :  
                                  --   NHMD Tracheophyta - 688130 
                                  --   NHMD Entomology   - 163841
                                  --   NHMA Entomology   - 32769
                                  --   NHMD Inv Palaeo   - 327682
