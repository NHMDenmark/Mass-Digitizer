...
SELECT CONCAT_WS(' | ', '"Natural History Museum of Denmark' , t1.Name , t2.name ) AS stor, stdi.name AS rank_name,
  CASE 
    WHEN t2.Name = 'Herbarium C: Danish Vascular Plant Collection'  THEN CONCAT('Box ', t3.Name)  
    WHEN t2.Name = 'Herbarium CP: Danish Vascular Plant Collection' THEN CONCAT('Shelf ', t3.Name) 
	 ELSE t3.Name
  END AS unit, 
