...
"""
  ...
  PURPOSE: Transform table with specify imports into a spreadsheet with author name drop-downs for each name row.
"""
import xlsxwriter
import pandas as pd
import sqlite3
...
storagetxt = ''
workbook = xlsxwriter.Workbook("authorDropdown001.xlsx") # Output Excel sheet and set name
worksheet = workbook.add_worksheet()


def createDF_fromSQL(con, sql, orderColumn, storageName):
    '''
...
speciesDf = createDF_fromSQL(con, speciesSQL, 'Box', 'box') # Make sure to order by the column that fits the workflow the best. I assume it is storage name (for Vascular plants= 'Box')
print('-------------------')
...
spAuthorList = speciesDf['authorlist'].to_list()
...
authorsToDropDowns(worksheet, speciesAuthorList, 'C')
...
print("############", genusDF.head(2))

print("¤¤¤¤¤¤¤¤", genusDF['authorlist'].head(2).to_string())

print('$$$$$$$$$$$$$$$$$$$')
# print(genusDF['authors'].head(2).to_string())
...
dfLength = lengthDataframe(speciesDf)

randomIndex = random.randint(1, dfLength)
randRow = speciesDf.iloc[[randomIndex]] # The random row to be tested on
print(f"At index {randomIndex}:: ", randRow.to_string())
print("The authors list:", randRow['authorlist'])
author_list_test = randRow['authorlist']
taxonName_for_test = list(randRow['name'])
print(taxonName_for_test, type(taxonName_for_test))
print(taxonName_for_test[0], '+', len(taxonName_for_test))
tst_name = taxonName_for_test[0]
pandasAuthors = list(randRow['authorlist'])[0]
print(pandasAuthors)

# sql_for_test = f'SELECT author FROM taxonname t WHERE t.name = {} AND length(author) > 0;'
# resultFromSQL = pd.read_sql_query(sql_for_test, con
#                                   )

# PLAN: Get a SQL row on the species name and test to see if the author is in the authorList
workbook.close()
